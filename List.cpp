...
Error_list_t CheckList(List_t* list, int anchor)
{
    ...
    {
        printf("\nError: size is lower than NULL!\n");
        return ERROR_SIZE;
    }
    else if (list->capacity < list->size)
    {
        printf("\nError: capacity is lower than size!\n");
        return ERROR_CAPACITY;
    }
    else if (anchor < 0)
    {
        printf("\nError: index of anchor is lower than NULL!\n");
        return ERROR_ANCHOR;
    }
    else if (anchor >= list->size)
    {
        printf("\nError: index of anchor is bigger than size!\n");
        return ERROR_ANCHOR;
    }
    else if (list->free <= 0)
    {
        printf("\nError: index of free element is lower than NULL!\n");
        return ERROR_FREE;
    }
    else if (list->free >= list->capacity)
    {
        printf("\nError: index of free element is bigger than capacity!\n");
        return ERROR_FREE;
    }
    return LIST_OK;
}
...
Error_list_t PoisonArray(int* array, int anchor)
{
    if (array == NULL)
    {
        printf("Error: address of poisoning array does not exist!\n");
        return ERROR_ADDRESS;
    }
